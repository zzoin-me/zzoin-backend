...
public class ProjectPreviewResponseDTO {
    ...
    private List<JobCategoryCode> categories;
    ...
    public static ProjectPreviewResponseDTO from(Project project, List<ProjectRecruitment> recruitments) {
        return ProjectPreviewResponseDTO.builder()
                ...
                .categories(recruitments.stream().map(x->x.getJobRole().getJobCategory().getCategoryCode()).distinct().toList())
                .recruitments(recruitments.stream().map(x->x.getJobRole().getName()).toList())
